...
export default function EditProduct({ product, categories }: EditProductProps) {
    const { data, setData, post, processing, errors, isDirty } = useForm({
        ...
        min_stock: product.min_stock !== undefined && product.min_stock !== null ? product.min_stock.toString() : '',
        sell_price: product.sell_price !== undefined && product.sell_price !== null ? product.sell_price.toString() : '',
        buy_price: product.buy_price !== undefined && product.buy_price !== null ? product.buy_price.toString() : '',
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                                    ...
                                    {data.photo ? (
                                        <div className="flex flex-col items-center">
                                            <div className="relative h-24 w-24 overflow-hidden rounded-xl border border-gray-200">
                                                <img src={URL.createObjectURL(data.photo)} alt="Preview Baru" className="h-full w-full object-cover" />
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center">
                                            <div className="relative h-24 w-24 overflow-hidden rounded-xl border border-gray-200 bg-white">
                                                <img src={`/storage/${product.photo}`} alt="Foto Saat Ini" className="h-full w-full object-cover" />
                                            </div>
                                            <p className="mt-4 text-sm font-medium text-gray-700 dark:text-gray-300">
                                                Foto saat ini
                                            </p>
                                            <p className="mt-1 text-xs text-gray-500">Klik untuk mengganti foto baru</p>
                                        </div>
                                    ) : (
                                        <>
                            ...
                            <div className="mt-4 flex items-center justify-end gap-3 pt-6">
                                <Button variant="ghost" type="button" asChild className="h-11 px-6" disabled={processing}>
                                    <Link href={dashboard.show(product.id)}>Batal</Link>
                                </Button>
                                <Button 
                                    type="submit" 
